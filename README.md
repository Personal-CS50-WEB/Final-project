...
As requested, I will go through the requested points one by one.

- **Justification of the distinctiveness and complexity**

   As it is a survey platform application where users can create, view, and participate in surveys.
   
   The following functional requirements justify the complexity of the project:
   ...
   The following points justify the distinctiveness of this project:

      - The architecture of the project is different then the course projects as it consists of two decoupled applications; a backend application and a frontend application. A set of REST APIs where defined to standardize the communication between the two applications.
      - The backend database has 14 tables with multi-level relations.
      - The survey model contains questions with dynamic model, as for each question type there is a different structure for the question record and the answer record.
      - Survey results display depends on the survey questions types. Bar charts were used to display radio button and checkbox and integer results while for text answers a list of responses were displayed.
      - In the frontend, when creating a new survey, user has to enter questions one by one and the wizard is dynamic as it responds to the user selected question type to display the correct set of input fields based on that type. All of the wizard is displayed in a single dynamic page which is more complex than what we have done in the other projects.
      - Security: 
         - User registration by email.
         - API authentication by JWT token that can be checked for expiration and can be refreshed as well.


 - **Utilizing Django including django models on the back-end and JavaScript on the front-end**: 

   The application consists of two applications that were developed independently:

   ### **1. Backend:**
   The backend app was developed using Django REST framework and using django models to create tables in database for each entity.
   ...
   - postgreSQL.
   ### **2. Frontend:**
   Frontend app was developed using React.js which is a java script framework.

 - **The web application is mobile-responsive**:
      - In the HTML page, I used Viewport meta tag to ensure the page is responsive to the screen width.
      `<meta name="viewport" content="width=device-width,initial-scale=1"/>`
      - Using Google Developer Tools, I used 'Toggle Device toolbar' feature to ensure the responsiveness of the frontend application.
...
Then, Django REST Framework was used to model those entities, apply business rules, develop and expose REST APIs.

### **Files** 
> This satisfies **the Readme requirement of describing each file in the project**.
   #### **Models**
 - Django Models are a level of abstraction on top of SQL that works with databases using Python classes and objects rather than direct SQL queries. Generally, each model maps to a single database table.Each attribute of the model represents a database field.
  ...
  - The serializers used in this project:
      ...
      **Inside serializers folder**

      - **userserializer.py**: Contains serializer class for user model.
      - **answerserializer.py**: Contains serializers classes for the answer types, text, integer and option answer in addition to answer serializers for answer model which contains the text answer, integer answer, option answers and question as childs in the nested serializer.
      - **questionserializer.py**: Contains serializers classes for option question and question class which is a nested dynamic serializer containing all question model fields           plus options and answers as childs.
      - **surveyserializer.py**: Contains serializer class for survey model which is a nested dynamic serializer containing questions field as a child and total submissions as a read only field.
  ...
  - Mapping operations on entities to views:
      ...
      **Inside views folder**
      ...
      - **survey.py**: Contains SurveyView: using DRF viewset class SurveyView is read only class that allows to list active surveys or retrieve one survey using SurveySerializer as serializer_class.
      - **submission.py**: Contains SubmissionView: using DRF mixins class SubmissionView extends mixins.CreateModelMixin and mixins.ListModelMixin classes that allows to create new record in Submission model and list submissions for one survey using SubmissionSerializer as serializer_class.
      -  **result.py**: Contains ExpiredSurveyViewSet: using DRF viewset class SurveyView is read only class that allows to list closed surveys or retrieve one survey using SurveyResultSerializer   as serializer_class.
      - **user_submission.py**: Contains UserSubmissionView: using DRF mixins class SubmissionView is read only class that allows to  list submissions for surveys the user submit and  retrieve one    submission using SubmissionSerializer as serializer_class and IsAuthenticated as permission_classes.
      ...
      - **types.py**: Contains TypesAPIView: view allows users to get question types.

### **APIs**  
> This satisfies **the Readme requirement of describing each file in the project**.

APIs were defined on on top of those views using the following files:
